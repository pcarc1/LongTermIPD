...
# 
# # Parameters
# N_years <- max(mod9_df$year_n) # Number of years
# N_sts <- ncol(mod9_mat) # Number of serotypes
# rho_beta1 <- 0.8  # Autocorrelation for beta1
# rho_beta2 <- 0.5  # Autocorrelation for beta2
# rho_eps <- 0.6    # Autocorrelation for epsilon
# 
# tau_global <- 1.5   # Precision for delta1 (serotype intercepts)
# tau_beta1 <- 0.2    # Precision for beta1 AR(1)
# tau_beta2 <- 0.2    # Precision for beta2 AR(1)
# tau_eps <- 0.3      # Precision for serotype-specific epsilon
# 
# # Group assignments
# grp <- rbinom(N_sts, 1, 0.5)
# 
# # Alpha1
# alpha1 <- rnorm(1, 0, 1)
# 
# # Serotype intercepts
# delta1 <- rnorm(N_sts, 0, 1 / sqrt(tau_global))
# 
# # Simulate AR(1) processes for beta1 and beta2
# beta1 <- numeric(N_years)
# beta2 <- numeric(N_years)
# beta1[1] <- rnorm(1, 0, sqrt((1 - rho_beta1^2) * tau_beta1))
# beta2[1] <- rnorm(1, 0, sqrt((1 - rho_beta2^2) * tau_beta2))
# 
# for (i in 2:N_years) {
#   beta1[i] <- rnorm(1, rho_beta1 * beta1[i - 1], 1 / sqrt(tau_beta1))
#   beta2[i] <- rnorm(1, rho_beta2 * beta2[i - 1], 1 / sqrt(tau_beta2))
# }
# 
# # Simulate AR(1) processes for serotype-specific effect epsilon1
# epsilon1 <- matrix(0, nrow = N_years, ncol = N_sts)
# epsilon1[1, ] <- rnorm(N_sts, 0, 1 / sqrt((1 - rho_eps^2) * tau_eps))  # Initial values
# 
# for (j in 1:N_sts) {
#   for (i in 2:N_years) {
#     epsilon1[i, j] <- rnorm(1, rho_eps * epsilon1[i-1, j], 1 / sqrt(tau_eps))
#   }
# }
# 
# # Simulate lambda
# lambda <- matrix(0, nrow = N_years, ncol = N_sts)
# 
# for (i in 1:N_years) {
#   for (j in 1:N_sts) {
#     # Two distinct groups with different beta1 and beta2 effects
#     if (grp[j] == 1) {
#       lambda[i, j] <- exp(alpha1 + delta1[j] + epsilon1[i, j] + beta1[i])
#     } else {
#       lambda[i, j] <- exp(alpha1 + delta1[j] + epsilon1[i, j] + beta2[i])
#     }
#   }
# }
# 
# # Simulate N_IPD
# r <- runif(N_sts, 1, 10)
# N_IPD <- matrix(0, nrow = N_years, ncol = N_sts)
# 
# for (i in 1:N_years) {
#   for (j in 1:N_sts) {
#     prob <- r[j] / (r[j] + lambda[i, j])
#     N_IPD[i, j] <- rnbinom(1, size = r[j], prob = prob)
#   }
# }
# 
# # Get dataframe
...
simulated_data |> 
  ...
  distinct() |> 
  mutate(group = if_else(group < 0.5, "Group 1 (beta1)", "Group 2 (beta 2)")) |> 
  group_by(group) |> 
