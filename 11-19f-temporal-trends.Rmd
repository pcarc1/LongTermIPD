...
db_19F_wide <- db_19F |> 
  ...
  mutate(baseline = case_when(!is.na(IPD_19F_2010) ~ IPD_19F_2010, 
                              !is.na(IPD_19F_2011) ~ IPD_19F_2011,
                              !is.na(IPD_19F_2012) ~ IPD_19F_2012),
         baseline_prop = case_when(!is.na(prop_19F_2010) ~ prop_19F_2010,
                                   !is.na(prop_19F_2011) ~ prop_19F_2011,
                                   !is.na(prop_19F_2012) ~ prop_19F_2012)) |> 
  rowwise() |> 
  mutate(comparison = if_else(!is.na(IPD_19F_2017), mean(IPD_19F_2017, IPD_19F_2018, IPD_19F_2019, na.rm = TRUE), IPD_19F_2014),
         comparison_prop = if_else(!is.na(prop_19F_2017), mean(prop_19F_2017, prop_19F_2018, prop_19F_2019, na.rm = TRUE), prop_19F_2014),
         ratio_cases = baseline/comparison,
         ratio_prop = baseline_prop/comparison_prop) |> 
  ungroup()
...
```{r, echo = FALSE}
reactable(data_19F |> 
            dplyr::select(country, year_pcv7, year_pcv10, year_pcv13, schedule, time_secondvax, mean_cov, baseline, baseline_prop, comparison, comparison_prop, ratio_cases, ratio_prop),
...
summary(lm(ratio_cases ~ vax_type, data = data_19F))

# Ratio of proportions
# PCV10 vs PCV13 vs both: not significant
summary(lm(ratio_prop ~ vax_type, data = data_19F))

# Ratio of cases
# Years between PCV7 and PCV10/13: not significant
summary(lm(ratio_cases ~ time_secondvax, data = data_19F))

# Ratio of proportions
# Years between PCV7 and PCV10/13: significant, -0.12
summary(lm(ratio_prop ~ time_secondvax, data = data_19F))

# Ratio of cases
# Mean vaccine coverage: not significant
summary(lm(ratio_cases ~ mean_cov, data = data_19F))

# Ratio of proportions
# Mean vaccine coverage: not significant
summary(lm(ratio_prop ~ mean_cov, data = data_19F))

# Ratio of cases
# Vaccination schedule: not significant
summary(lm(ratio_cases ~ schedule, data = data_19F))

# Ratio of proportions
# Vaccination schedule: not significant
summary(lm(ratio_prop ~ schedule, data = data_19F))

# Ratio of cases
# None significant
summary(lm(ratio_cases ~ vax_type + time_secondvax + mean_cov + schedule, data = data_19F))

# Ratio of proportions
# None significant
summary(lm(ratio_prop ~ vax_type + time_secondvax + mean_cov + schedule, data = data_19F))
```
