...
# ts_data <- pcv13_norm |> 
#   group_by(st) |> 
#   summarise(N_IPDst = list(N_IPDst)) |> 
#   pull(N_IPDst, name = st)
```
...
for (i in names(pcv13)) {
  gmods[[i]] <- gam(N_IPD ~ s(x = nyear, bs = "ps", k = 25),
                      family = poisson,
                      method = "REML",
...
for (i in names(pcv13)){
  smooth_pcv13[[i]] = data.frame(fitcases = gmods[[i]]$fitted.values) %>% 
    dplyr::mutate(nyear = seq.int(from = 1, by = 1, length.out = n()))
}

ts_data <- smooth_pcv13 |>
  enframe() |> 
  unnest(cols = c(value)) |> 
  group_by(name) |> 
